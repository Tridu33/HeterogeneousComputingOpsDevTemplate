...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z)
    {
        ...
        //       主机侧全局数据指针 指向外部存储的数据起始地址，BLOCK_LENGTH是glocalTensor所包含的全局数据需要自行在CPU数据切分算法中保证不会超过长度
...
void add_custom_do(uint32_t blockDim, void* l2ctrl, void* stream, uint8_t* x, uint8_t* y, uint8_t* z)
{
    ...
    // add_custom_do核函数调用 在blockDim张卡上跑GetBlockIdx()获取ID l2ctrl是保留参数 任务流stream管理任务的并行 
}
#endif
